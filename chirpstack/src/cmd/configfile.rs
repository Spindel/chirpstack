...
# Message Logger configuration (optional)
[message_logger]
  
  [backend.mqtt]
  # Topic to post logs to
  log_topic = "{{ message_logger.backend.mqtt.log_topic }}"

  # A list of MQTT servers (e.g. scheme://host:port where scheme is tcp, ssl or ws)
  servers = [
  {{#each message_logger.backend.mqtt.servers}}
  "{{this}}",
  {{/each}}
  ]

  # Connect with the given username (optional)
  username="{{ message_logger.backend.mqtt.username }}"

  # Connect with the given password (optional)
  password="{{ message_logger.backend.mqtt.password }}"

  # Keep alive interval.
  #
  # This defines the maximum time that that should pass without communication
  # between the client and server.
  keep_alive_interval="{{ message_logger.backend.mqtt.keep_alive_interval }}"
  ...
  # Note: an increase of this value will decrease the performance.
  # For more information: https://www.hivemq.com/blog/mqtt-essentials-part-6-mqtt-quality-of-service-levels
  qos={{ message_logger.backend.mqtt.qos }}
  ...
  # connects to an MQTT broker. By setting this flag you are indicating
  # that no messages saved by the broker for this client should be delivered.
  clean_session={{ message_logger.backend.mqtt.clean_session }}
  ...
  # broker. A client id must be no longer than 23 characters. If left blank,
  # a random id will be generated by ChirpStack.
  client_id="{{ message_logger.backend.mqtt.client_id }}"
  ...
  # but the certificate used by the server is not trusted by any CA certificate
  # on the server (e.g. when self generated).
  ca_cert="{{ message_logger.backend.mqtt.ca_cert }}"

  # TLS certificate file (optional)
  tls_cert="{{ message_logger.backend.mqtt.tls_cert }}"

  # TLS key file (optional)
  tls_key="{{ message_logger.backend.mqtt.tls_key }}"
